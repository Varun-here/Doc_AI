...
function App() {
  ...
  const getApiUrl = () => {
    if (window.location.hostname === "localhost") {
      return "http://localhost:5000/api/documents/analyze";
    }

    return "/_/backend/api/documents/analyze";
  };
    ...
    <section id="upload" className="mx-auto max-w-7xl px-6 py-16">
      ...
      <div className="glass-card mx-auto max-w-4xl rounded-[2rem] p-6 md:p-8">
        ...
        >
          ...
          <h3 className="text-2xl font-bold">Upload a document for analysis</h3>

          <p className="mt-3 max-w-lg text-slate-300">
            The platform reads your file and produces a structured report with
            the document type, summary, extracted details, missing fields,
            risks, recommended actions, and visual PDF highlights.
          </p>
    ...
    <div className="space-y-5">
      ...
      {highlightTerms.length === 0 && (
        <p className="text-sm text-amber-200">
          No extracted values were available for visual highlighting. The PDF
          can still be viewed above.
        </p>
      )}
    </div>
  );
